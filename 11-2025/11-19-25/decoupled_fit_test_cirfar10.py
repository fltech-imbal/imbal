...
for i in range(num_classes):
    if FILTER == 'binary':
        ...
        if i == 0:
            ...
            if i < 5:
                continue
            x_train_filter.append(np.ones(x_train[y_train == i][:400].shape))
            y_train_filter.append(np.ones(y_train[y_train == i][:400].shape))
            x_test_filter.append(np.ones(x_test[y_test == i].shape))
            y_test_filter.append(np.ones(y_test[y_test == i].shape))
