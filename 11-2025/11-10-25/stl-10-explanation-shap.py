...
for i in range(EXPLAIN_AMOUNT):
    ...
    # print(x_.shape)
    # print(x_.dtype)
    # masker = shap.maskers.Image("inpaint_telea", (96, 96, 3))
    # explainer = shap.Explainer(f, masker, output_names=class_labels)
    #
    # shap_values = explainer(x_1, max_evals=500, batch_size=1, outputs=[y_])
    # shap.image_plot(shap_values)

    # imbal.classification.lime_image_explanation(
    #     x_,
    #     model,
    #     actual_label=y_,
    #     # label_to_explain=y_,
    ...
    masker = shap.maskers.Image("blur(128, 128)", (96, 96, 3))

    explainer = shap.Explainer(model, masker,
                               output_names=class_labels)
    # shap_values = explainer(np.expand_dims(x_, axis=0), max_evals=200, batch_size=50, outputs=shap.Explanation.argsort.flip[:1])
    shap_values = explainer(np.expand_dims(x_, axis=0), max_evals=200, batch_size=50)

    shap.image_plot(shap_values, np.expand_dims(x_, axis=0))
