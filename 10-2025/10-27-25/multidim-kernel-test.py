...
x = np.linspace(-3, 3, spacing//2)[:-1] + 6/spacing
y = np.linspace(-3, 3, spacing//2)[:-1] + 6/spacing
xx, yy = np.meshgrid(x, y)
half_grid = np.vstack([xx.ravel(), yy.ravel()]).T

x_edges = np.linspace(-3, 3, spacing//2)
y_edges = np.linspace(-3, 3, spacing//2)

fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(18, 6))

counts, xedges, yedges, image = ax[0].hist2d(data[:, 0], data[:, 1], cmap='viridis', bins=[x_edges, y_edges], density=True)
...
ax[0].set_title('3D Kernel Density Estimate Surface')
...
sc = ax[1].scatter(half_grid[:, 0], half_grid[:, 1], c=less_densities, cmap='viridis', marker='s', s=120)
...
ax[1].set_ylim(-3, 3)
# ax.set_zlabel('Density')
ax[1].set_title('3D Kernel Density Estimate Surface')
# ax.view_init(elev=35, azim=135)  # tweak for a nice angle
