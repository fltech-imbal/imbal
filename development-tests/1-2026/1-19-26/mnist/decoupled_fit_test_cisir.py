...
y_test_filter = []
for i in range(num_classes):
    x_train_filter.append(x_train[y_train == i][:round(60*1.65**(9-i))])
    y_train_filter.append(y_train[y_train == i][:round(60*1.65**(9-i))])
    x_test_filter.append(x_test[y_test == i][:round(10*1.65**(9-i))])
    y_test_filter.append(y_test[y_test == i][:round(10*1.65**(9-i))])
