...
# ----------------------------
# DenseLoss / DenseWeight
# ----------------------------
def denseweight_from_densities(densities, alpha=1.0, epsilon=1e-6):
    densities = np.asarray(densities).reshape(-1)

    density_min = np.min(densities)
    density_max = np.max(densities)

    normalized_densities = (densities - density_min) / (
        density_max - density_min + 1e-8
    )

    raw_weights = np.maximum(
        1.0 - alpha * normalized_densities,
        epsilon
    )

    dense_weights = raw_weights / np.mean(raw_weights)

    return dense_weights.astype("float32")
...
MODEL_SAVE_PATH = "saved_models/denseloss-fit-model-val-ae.keras"
...
if LOAD_SAVED_MODEL and os.path.exists(MODEL_SAVE_PATH):
    print(f"Loading saved DenseLoss regression model from {MODEL_SAVE_PATH}")
    ...
    # ----------------------------
    # DenseLoss weight candidates
    # ----------------------------
    weight_candidates = np.array([
        denseweight_from_densities(
            densities,
            alpha=alpha
        )
        for alpha in dense_loss_alphas
    ]).astype("float32")
    ...
    if best_alpha_index is not None:
        best_alpha = dense_loss_alphas[best_alpha_index]
