...
def loss_fn(y_true, y_pred):
    return keras.losses.mean_squared_error(y_true, y_pred) + 0.1*pcc(y_true, y_pred)
...
if VALIDATION_DATA:
    ...
    sample_weights = denseweight(sample_densities, alpha=[0.1*(i+1) for i in range(10)] if WEIGHT_CANDIDATES else 1)
    val_densities = imbal.regression.get_sample_densities(
        y_val,
        kde_bandwidth,
        distribution=y_train
    )
    w_val = denseweight(val_densities, alpha=[0.1*(i+1) for i in range(10)] if WEIGHT_CANDIDATES else 1)
...
model.save(f"models/{DATA_PATH}_{FIT.name.lower()}_{'w' if VALIDATION_DATA or AE else ''}{'_validation' if VALIDATION_DATA else ''}{'_ae' if AE else ''}{'_third_last' if AE_THIRD_TO_LAST and AE else ''}_dw_pcc.keras")
...
imbal.regression.plot_true_vs_predictions(
    ...
    save_figure=f"results/{DATA_PATH}_{FIT.name.lower()}_{'w' if VALIDATION_DATA or AE else ''}{'_validation' if VALIDATION_DATA else ''}{'_ae' if AE else ''}{'_third_last' if AE_THIRD_TO_LAST and AE else ''}_dw_pcc.png"
)