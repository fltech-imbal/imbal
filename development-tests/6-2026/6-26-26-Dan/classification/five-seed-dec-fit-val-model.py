...
BEST_PARAMS_SAVE_PATH = "saved_models/best_params_decoupled_fit-val.json"
...
if best_model is not None:
    ...
    with open(BEST_PARAMS_SAVE_PATH, "w") as f:
        json.dump({
            "best_weight_index": (
                int(best_params["best_weight_index"])
                if best_params["best_weight_index"] is not None
                else None
            ),
            "best_class_weights": (
                [float(x) for x in best_params["best_class_weights"]]
                if best_params["best_class_weights"] is not None
                else None
            ),
            "best_decision_threshold": (
