...
def generate_ae_branch(
    ...
    for block in ae_blocks:
        print('----- BLOCK -----')
        for layer in block:
            print(f'\t{layer}, {layer.input.shape}, {layer.output.shape}')
            print(f'\t\t{layer.get_config()}')
    ...
    ae_extended_model = keras.models.Model(inputs=model.inputs, outputs=model.outputs + [ae_branch_blocks[-1][-1].output])
...
# (32,32,1)
# warn
# (32,32) --> Flatten --> Dense(32*32*3) --> Reshape((32,32,3))
...
extended_model, ae_branch_layers = generate_ae_branch(model, representation_layer_index=-5)
...
parameters = imbal.classification.wrap_model_compile_parameters(
    loss=["categorical_crossentropy", mse_reconstruction_loss],
