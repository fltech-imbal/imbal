...
reset_layers.append(output)

# Hand-crafted AE
if AE == 'hand':
    y = layers.Reshape((4, 4, 64))(latent)
    stop_layers.append(y)
    ...
    stop_layers.append(input_recon)
elif AE == 'mirror':
    # True mirror AE
    y = layers.Reshape((4, 4, 64))(latent)
...
def mse_reconstruction_loss(y_true, y_pred):
    ...
    # Generalize for any input type/shape (image, tabular (1D) data, etc.)
    loss_per_example = tf.reduce_mean(sq, axis=[1,2,3])
