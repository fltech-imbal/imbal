...
MEDIAN_PARAMS_SAVE_PATH = (
    "saved_models/median_params_regular_fit_regression-val-ae-rep-3.json"
)
MULTI_SEED_RESULTS_SAVE_PATH = (
    "saved_results/multi_seed_results_regular_fit_regression-val-ae-rep-3.json"
)
...
for seed in seeds:
    ...
    temporary_model_path = os.path.join(
        TEMP_MODEL_DIR,
        f"regular_fit_val_ae_rep_3_seed_{seed}.keras",
    )
...
# Remove only this script's temporary model files.
...
# Remove the shared directory only if no other temporary files remain.
if (
