...
MEDIAN_PARAMS_SAVE_PATH = (
    "saved_models/median_params_regular_fit_regression-val-ae.json"
)
MULTI_SEED_RESULTS_SAVE_PATH = (
    "saved_results/multi_seed_results_regular_fit_regression-val-ae.json"
)

# Shared temporary directory naming scheme used by the earlier five-seed
# validation + autoencoder scripts. The filenames remain configuration-specific.
TEMP_MODEL_DIR = "saved_models/five_seed_temp_models_val_ae"
...
for seed in seeds:
    ...
    # Fresh copies are required so each seed starts with the full dataset.
    ...
    rare_mae = float(
        np.mean(
            np.abs(
                y_true[rare_mask]
                - predictions[rare_mask]
            )
        )
    )

    aore = float(
        (common_mae + rare_mae) / 2.0
    )

    temporary_model_path = os.path.join(
        TEMP_MODEL_DIR,
        f"regular_fit_val_ae_seed_{seed}.keras",
    )
...
# Remove only this script's temporary files. Other configurations using the
# shared temporary folder are left untouched.
...
# Remove the shared directory only if it is empty.
if (
    os.path.exists(TEMP_MODEL_DIR)
    and not os.listdir(TEMP_MODEL_DIR)
):
