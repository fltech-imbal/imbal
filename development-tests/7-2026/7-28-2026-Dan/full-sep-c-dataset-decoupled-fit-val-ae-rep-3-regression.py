...
# ----------------------------
# False Positives on Pseudolabels
# ----------------------------
threshold = np.log(10)

# Actual pseudolabeled samples (< 0)
pseudolabel_mask = y_true < 0

# Predicted as rare (> ln(10))
predicted_positive_mask = y_pred > threshold

# False positives:
# actual pseudolabel (<0) but predicted > ln(10)
false_positive_mask = pseudolabel_mask & predicted_positive_mask

false_positive_count = np.sum(false_positive_mask)

print(f"False Positives (actual < 0, predicted > ln(10)): {false_positive_count}")
