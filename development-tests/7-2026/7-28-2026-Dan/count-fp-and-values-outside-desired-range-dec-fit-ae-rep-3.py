...
MODEL_SAVE_PATH = "saved_models/decoupled-fit-model-val-ae-rep-3.keras"
...
if LOAD_SAVED_MODEL and os.path.exists(MODEL_SAVE_PATH):
    ...
    (x_train, y_train, sw_candidates), (x_val, y_val, sw_val) = imbal.regression.split(x_train, y_train,
    ...
    with open("saved_models/best_params_decoupled_fit_regression-val-ae-rep-3.json", "w") as f:
        json.dump({
...
num_pseudolabels_outside = int(np.sum(pseudolabel_mask & outside_range_mask))

false_positive_mask = pseudolabel_mask & (y_pred >= threshold)
false_positive_count = int(np.sum(false_positive_mask))

print(f"Number of pseudolabels predicted outside range: {num_pseudolabels_outside}")
