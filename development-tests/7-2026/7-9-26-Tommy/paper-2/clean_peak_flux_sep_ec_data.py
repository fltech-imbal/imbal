...
LOG_LABELS = True
EPSILON = 1e-5
ORIGINAL_DATA_PATH = "peak-SEP-EC-data"
USE_DELTA = False
...
if LOG_LABELS:
    training_labels = np.log(training_labels + EPSILON)
    test_labels = np.log(test_labels + EPSILON)
    val_labels = np.log(val_labels + EPSILON)
...
training.to_csv(f'cleaned-peak-flux-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_training.csv', index=False)
val.to_csv(f'cleaned-peak-flux-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_validation.csv', index=False)
test.to_csv(f'cleaned-peak-flux-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_test.csv', index=False)