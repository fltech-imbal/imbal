...
LOG = True
EPSILON = 1e-9
ORIGINAL_DATA_PATH = "data/SEP-EC"
...
training.to_csv(f'cleaned-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_training.csv', index=False)
val.to_csv(f'cleaned-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_validation.csv', index=False)
test.to_csv(f'cleaned-SEP-EC-data/sep_e{"c" if INCLUDE_CME else ""}{"" if INCLUDE_PROTON else "_no_proton"}{"" if INCLUDE_ELECTRON else "_no_electron"}{"_log" if LOG else ""}{"_normalized" if NORMALIZED else ""}_test.csv', index=False)