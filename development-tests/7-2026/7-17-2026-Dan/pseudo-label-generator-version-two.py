...
6. Repeat until TARGET_ACCEPTED_PERCENT of the original unknown samples have
   been accepted, or until a safety stopping condition is reached.
...
INITIAL_PARAMS_PATH = (
    SCRIPT_DIRECTORY
    / "saved_models/best_params_pseudo-label-generator.json"
)

# Final artifacts produced by this script.
FINAL_MODEL_PATH = (
    SCRIPT_DIRECTORY / "saved_models/pseudo-label-version-two-model.keras"
)
FINAL_PARAMS_PATH = (
    SCRIPT_DIRECTORY / "saved_models/pseudo-label-version-two-params.json"
)
OUTPUT_CSV_PATH = (
    SCRIPT_DIRECTORY / "iterative_predicted_ln_peak_intensity.csv"
)
...
# Stop after this cumulative percentage of the original unknown samples has
# been accepted and added to training. Example: 80.0 means 80 percent.
TARGET_ACCEPTED_PERCENT = 80.0
...
def validate_configuration() -> None:
    ...
    if not 0.0 < TARGET_ACCEPTED_PERCENT <= 100.0:
        raise ValueError(
            "TARGET_ACCEPTED_PERCENT must be greater than 0 and at most 100."
        )
...
def min_max_scale_to_desired_range(predictions: np.ndarray) -> np.ndarray:
    """Scale the full prediction vector into [DESIRED_MIN, DESIRED_MAX]."""
    source_min = float(np.min(predictions))
    source_max = float(np.max(predictions))

    if np.isclose(source_min, source_max):
        midpoint = (DESIRED_MIN + DESIRED_MAX) / 2.0
        print(
            "All final raw predictions are equal; assigning every scaled "
            f"prediction to the desired-range midpoint {midpoint:.6f}."
        )
        return np.full(predictions.shape, midpoint, dtype=np.float64)

    normalized = (predictions - source_min) / (source_max - source_min)
    return DESIRED_MIN + normalized * (DESIRED_MAX - DESIRED_MIN)
...
def main() -> None:
    ...
    required_accepted = int(
        np.ceil(total_unknown * TARGET_ACCEPTED_PERCENT / 100.0)
    )
    ...
    print(
        f"Target accepted count: {required_accepted}/{total_unknown} "
        f"({TARGET_ACCEPTED_PERCENT:.2f}%)"
    )

    for iteration in range(1, MAX_ITERATIONS + 1):
        if len(accepted_indices) >= required_accepted:
            stopping_reason = "target accepted percentage reached"
            break
        ...
        if new_count < MIN_NEW_SAMPLES_PER_ITERATION:
            stopping_reason = (
                "no sufficient new acceptable pseudo-labels were found; "
        ...
        # Retrain even when this batch reaches the target, so the final model
        # incorporates every pseudo-label accepted in the final iteration.
        print(
    ...
    if accepted_count < required_accepted:
        print(
            "Warning: the requested accepted percentage was not reached. "
            "The script will still generate and scale final predictions using "
    ...
    save_run_parameters(
        {
            ...
            "target_accepted_percent": TARGET_ACCEPTED_PERCENT,
