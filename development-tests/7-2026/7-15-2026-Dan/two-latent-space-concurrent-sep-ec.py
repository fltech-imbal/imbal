...
def build_concurrent_representation_model(input_count):
    """
    Learn two independent latent spaces from the same raw input.

    The original branch is optimized with uniform sample weights.
    The density-aware branch is optimized with tail-aware sample weights.
    Auxiliary heads make it possible to apply different weighting to the two
    branches while training them concurrently.
    """
    ...
    return keras.Model(
        ...
        name="concurrent_representation_model",
    )
...
def create_concurrent_sample_weights(tail_labels, density_tail_weight):
    tail_labels = np.asarray(tail_labels).reshape(-1)

    uniform_weights = np.ones_like(tail_labels, dtype="float32")
    density_weights = np.ones_like(tail_labels, dtype="float32")
    density_weights[tail_labels == 1] = density_tail_weight

    return {
        "original_regression_output": uniform_weights,
        "original_classification_output": uniform_weights,
        "density_regression_output": density_weights,
        "density_classification_output": density_weights,
    }
...
def build_fused_regressor(
    ...
    """
    Freeze the two learned encoders and train one regressor on their combined
    representations. Concatenation lets the regressor choose features from
    either latent space through its learned downstream weights.
    """
...
def run_concurrent_two_latent_training(seed=42):
    ...
    best_density_tail_weight = None
    best_representation_epoch_count = None
    best_density_branch_validation_aore = np.inf

    for density_tail_weight in candidate_tail_weights:
        optimal_epoch_count = (
            get_concurrent_representation_epoch_count_with_kfold(
                training_features,
                training_regression_targets,
                training_tail_labels,
                density_tail_weight=density_tail_weight,
                fold_count=5,
                seed=seed,
            )
        )
        ...
        print(
            ...
            f"density weight={density_tail_weight}, "
            f"optimal epochs={optimal_epoch_count}, "
            f"density-branch validation AORE={candidate_aore:.4f}"
        )

        if candidate_aore < best_density_branch_validation_aore:
            best_density_branch_validation_aore = candidate_aore
            best_representation_model = candidate_model
            best_density_tail_weight = density_tail_weight
            best_representation_epoch_count = optimal_epoch_count

    print("\nBest Concurrent Representation Configuration")
    print(f"Density-aware tail weight: {best_density_tail_weight}")
    print(f"Optimal epoch count: {best_representation_epoch_count}")
    print(
        "Density branch validation AORE: "
        f"{best_density_branch_validation_aore:.4f}"
    )
    ...
    for regressor_tail_weight in candidate_tail_weights:
        ...
        print(
            "Fused regressor "
            f"tail weight={regressor_tail_weight}, "
            f"optimal epochs={optimal_epoch_count}, "
        ...
        if candidate_validation_aore < best_validation_aore:
            ...
            best_regressor_tail_weight = regressor_tail_weight
    ...
    print(f"Regressor tail weight: {best_regressor_tail_weight}")
...
if __name__ == "__main__":
    final_model = run_concurrent_two_latent_training(seed=42)
