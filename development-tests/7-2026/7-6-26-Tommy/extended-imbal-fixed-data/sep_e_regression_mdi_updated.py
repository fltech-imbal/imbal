...
def mdi_importance(densities, alpha=1):
    ...
    if isinstance(alpha, list):
        return np.array([(1 - normalized_densities**a)**(1/a) for a in alpha])
    else:
        return (1 - normalized_densities**alpha)**(1/alpha)
...
if VALIDATION_DATA:
    ...
    sample_weights = mdi_importance(sample_densities, alpha=[0.25*(i+1) for i in range(10)] if WEIGHT_CANDIDATES else 1)
    val_densities = imbal.regression.get_sample_densities(
        y_val,
        kde_bandwidth,
        distribution=y_train
    )
    w_val = mdi_importance(val_densities, alpha=[0.25*(i+1) for i in range(10)] if WEIGHT_CANDIDATES else 1)
    ...
    sample_weights = mdi_importance(sample_densities)
...
model.save(f"models/{DATA_PATH}_{FIT.name.lower()}_{'w' if VALIDATION_DATA or AE else ''}{'_validation' if VALIDATION_DATA else ''}{'_ae' if AE else ''}{'_third_last' if AE_THIRD_TO_LAST and AE else ''}.keras")


imbal.regression.plot_true_vs_predictions(
    y_test,
    predictions,
    title=f'Common MAE: {common_mae:.4f}, Rare MAE: {rare_mae:.4f}, AORE: {(mae + rare_mae)/2:.4f}{f", Alpha: {[0.25*(i+1) for i in range(10)][model.best_weight_index]:.1f}" if WEIGHT_CANDIDATES else ""}',
    save_figure=f"results/{DATA_PATH}_{FIT.name.lower()}_{'w' if VALIDATION_DATA or AE else ''}{'_validation' if VALIDATION_DATA else ''}{'_ae' if AE else ''}{'_third_last' if AE_THIRD_TO_LAST and AE else ''}_mdi.png"
)