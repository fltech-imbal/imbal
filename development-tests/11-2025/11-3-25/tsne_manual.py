...
latents = intermediate_model.predict(x_test)

import matplotlib.pyplot as plt
from sklearn.manifold import TSNE

tsne = TSNE(n_components=2, random_state=None, perplexity=30)
...
for i in range(len(unique_classes)):
    scatter = ax.scatter(
        tsne_fit[:, 0][y_test == unique_classes[i]],
        tsne_fit[:, 1][y_test == unique_classes[i]],
        cmap='plasma',
        c=y_test[y_test == unique_classes[i]],
        vmin=label_min[0],
        vmax=label_max[0]
    )
ax.legend()
...
scatter = None
unique_classes, counts = np.unique(y_test, return_counts=True)
index_ordering = np.argsort(counts)
unique_classes = unique_classes[index_ordering]
...
plt.savefig(f'tsne-{MODE}-imbalanced-{IMBALANCED}_bad.png')
plt.show()
