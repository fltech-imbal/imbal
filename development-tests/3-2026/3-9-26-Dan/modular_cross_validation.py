...
def clone_convergence_stopping(cb):
    return cb.__class__(
        ...
        best_weight_identifier=getattr(cb, "best_weight_identifier", None),
    )
...
def _reset_metric_objects(metrics: List[keras.metrics.Metric]) -> None:
    for m in metrics:
        m.reset_state()


def _evaluate_metric_objects(
    model: keras.Model,
    ...
    for m in metrics:
        ...
        if sample_weight is None:
            ...
            m.update_state(y_val, y_pred, sample_weight=sample_weight)
        out[m.name] = float(m.result().numpy())
    return out
...
@dataclass
class DecoupledFitStageTwoExtraLayerStrategy(BaseStrategy):
    def make_call(self, model: Any) -> Callable[[Dict[str, Any]], Any]:
        return lambda fit_kwargs: decoupled_fit_with_cross_val_stage_two(model, **fit_kwargs)
    ...
    def model_prepare(self, model: Any) -> Any:
        ...
        new_model._generate_decoder_branch = getattr(model, "_generate_decoder_branch", False)
        new_model._use_decoder_branch = False
        new_model._decoder_branch = None
        new_model._extended_model = new_model
...
def fit_k_folds_modular(
    ...
    *,
    ...
    Selection rule:
      - For each candidate: score = mean over folds of best(metric)
      - Choose candidate with best score according to min_or_max
    ...
    If do_threshold_sweep=True:
      - On each fold, run an F1 threshold sweep on the validation predictions
      - Overrides selection_metric to F1_Score
      - Average the best thresholds across folds for each candidate
      - Return the selected candidate's averaged threshold in selection_info
    """
    ...
    if selection_min_or_max is None:
        selection_min_or_max = min_or_max
    ...
    if isinstance(strategy, DecoupledFitStageTwoExtraLayerStrategy):
        compile_cfg["generate_decoder_branch"] = False
        # compile_cfg["use_decoder_branch"] = False

    fit_call = strategy.make_call(model)

    allowed_for_sweep = isinstance(strategy, (BalancedFitStrategy, DecoupledFitStageTwoExtraLayerStrategy, DecoupledFitStageTwoStrategy))
    ...
    best_cv_score: float = float("inf") if selection_min_or_max == "min" else -float("inf")
    best_avg_best_epoch: int = 1
    best_avg_threshold: Optional[float] = None
    ...
    for cand_name, cand_params, cand_meta in candidates:
        ...
        fold_selection_values: List[float] = []
        ...
        for fold, (tr_idx, va_idx) in enumerate(
            ...
            X_va = x[va_idx]
            y_va = y[va_idx]
            sw_va = cand_params.sample_weight[va_idx] if cand_params.sample_weight is not None else None
            ...
            if selection_metric == metric:
                fold_selection_value = float(metric_values[best_epoch_idx])
            else:
                if selection_metric not in snap:
                    raise ValueError(
                        f"selection_metric='{selection_metric}' was not found in evaluated fold metrics. "
                        f"Available keys: {list(snap.keys())}"
                    )
                fold_selection_value = float(snap[selection_metric])

            fold_selection_values.append(fold_selection_value)

            if do_threshold_sweep:
                y_prob_va = model.predict(X_va, verbose=0)
                best_threshold, best_f1, _, _ = f1_threshold_sweep(
                    y_prob_va,
                    y_va,
                    step=0.1,
                    verbose=0,
                )
                fold_best_thresholds.append(float(best_threshold))
                fold_selection_values.pop()
                fold_selection_values.append(float(best_f1))

        avg_best_epoch = int(np.rint(np.mean(fold_best_epochs))) if fold_best_epochs else 1
        avg_best_epoch = max(1, avg_best_epoch)

        if fold_selection_values:
            cv_score = float(np.mean(fold_selection_values))
        else:
            cv_score = float("inf") if min_or_max == "min" else -float("inf")
        ...
        avg_threshold = None
        if do_threshold_sweep and fold_best_thresholds:
            avg_threshold = float(np.mean(fold_best_thresholds))

        print(
            f"[CV] candidate={cand_name} | mean_best_{metric}={cv_score:.6f} | avg_best_epoch={avg_best_epoch}"
            + (f" | avg_best_threshold={avg_threshold:.6f}" if avg_threshold is not None else "")
        )
        ...
        cv_by_candidate[cand_name] = {
            ...
            "avg_best_threshold": avg_threshold,
            "fold_best_thresholds": fold_best_thresholds,
        }

        if selection_min_or_max == "min":
        ...
        if is_better:
            ...
            best_avg_threshold = avg_threshold

    print(f"Selected candidate: {best_name} (mean_best_metric_value={best_cv_score:.4f})")
    print("Best epoch (average):", best_avg_best_epoch)
    if do_threshold_sweep and best_avg_threshold is not None:
        print("Best threshold (average across folds):", best_avg_threshold)
    ...
    selection_info = {
        ...
        "selected_threshold": best_avg_threshold,
