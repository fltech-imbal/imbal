...
if FIT != FitType.REGULAR:
    ...
    weights = [
        imbal.regression.reciprocal_importance(densities, alpha=0.1),
        imbal.regression.reciprocal_importance(densities, alpha=0.2),
        imbal.regression.reciprocal_importance(densities, alpha=0.3),
    ]
...
imbal.regression.tsne_visualization(
    ...
    y_test,
    representation_layer_index=REPRESENTATION_LAYER_INDEX,
    save_figure=f'{OUTPUT_PATH}/extra-tsne_visualization-{FIT.value}-ae-{AE}-rep{REPRESENTATION_LAYER_INDEX}-weights-{DENSITY_TO_WEIGHT_FUNCTION.__name__}-{DENSITY_TO_WEIGHT_KWARGS["alpha"]}-frozen-{FREEZE_SECOND_STAGE_EXTENDED}-wpcc-{USE_WPCC}.png' if GEN_OUTPUT else None,
)

tools.plot_true_vs_predictions(
    y_test,
    predictions,
    save_figure=f'{OUTPUT_PATH}/extra-regression-true-pred-{FIT.value}-ae-{AE}-rep{REPRESENTATION_LAYER_INDEX}-weights-{DENSITY_TO_WEIGHT_FUNCTION.__name__}-{DENSITY_TO_WEIGHT_KWARGS["alpha"]}-frozen-{FREEZE_SECOND_STAGE_EXTENDED}-wpcc-{USE_WPCC}.png' if GEN_OUTPUT else None,
)
