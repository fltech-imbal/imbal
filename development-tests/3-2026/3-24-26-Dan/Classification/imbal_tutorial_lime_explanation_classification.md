...
A key detail is that the **largest contribution (≈ 0.08) comes from `CME_DONKI_speed_norm`, and it points toward Common**. This is important because LIME is approximating the model *locally* around this instance. In that local neighborhood, increasing the speed feature would actually make the prediction *more* likely to be **Common**, which is counterintuitive given that higher speed is generally associated with Rare events.

This indicates that the **local decision boundary around this sample is tilted toward Common**, meaning nearby perturbed samples generated by LIME resemble Common-class behavior more than Rare.

A few things stand out:

* `CME_DONKI_speed_norm = 0.34` is relatively low compared to the correct example (`0.78`), and its local weight pushes toward **Common**, making it the dominant negative factor against Rare.
* `DONKI_half_width_norm = 0.75` contributes toward **Common** and reinforces that local trend.
* `CME_CDAW_MPA_norm = 0.20` also adds to the Common-side evidence.
* Although `Halo = 1.00` still supports **Rare**, its contribution is smaller relative to the dominant Common-driving features.

Overall, the model predicts **Common primarily because of the negative impact from speed in this local region**, combined with supporting contributions from other features. This highlights a key insight from LIME: **even if a feature is globally associated with a class, its local effect can reverse depending on the surrounding feature values and neighborhood structure**.
