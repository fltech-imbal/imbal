...
def plot_representation_space_3d(
    ...
    min_rep_x = representation_vectors[:, dim_one_index][dim_one_sort_order[5]]
    max_rep_x = representation_vectors[:, dim_one_index][dim_one_sort_order[-5]]

    min_rep_y = representation_vectors[:, dim_two_index][dim_two_sort_order[5]]
    max_rep_y = representation_vectors[:, dim_two_index][dim_two_sort_order[-5]]

    min_rep_z = representation_vectors[:, dim_three_index][dim_three_sort_order[5]]
    max_rep_z = representation_vectors[:, dim_three_index][dim_three_sort_order[-5]]

    # Plot central labels last
    abs_labels = np.abs(labels - float(np.median(labels)))
    sort_indices = np.argsort(abs_labels)
    ...
    scatter = ax.scatter(
        plot_sorted_representations[:, dim_one_index],
        plot_sorted_representations[:, dim_two_index],
        plot_sorted_representations[:, dim_three_index],
        c=plot_sorted_labels,
        cmap="jet",
        vmin=vmin,
        vmax=vmax,
