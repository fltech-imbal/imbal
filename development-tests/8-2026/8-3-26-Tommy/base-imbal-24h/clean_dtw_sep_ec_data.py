...
if USE_DELTA:
    ...
    training_labels = np.log(training_data[TARGET_NAME] + LABEL_EPSILON) - np.log(training_data['p_t'] + EPSILON)
    training_labels.name = 'delta_log_Intensity'
    val_labels = np.log(val_data[TARGET_NAME] + LABEL_EPSILON) - np.log(val_data['p_t'] + EPSILON)
    val_labels.name = 'delta_log_Intensity'
    test_labels = np.log(test_data[TARGET_NAME] + LABEL_EPSILON) - np.log(test_data['p_t'] + EPSILON)
...
#     val_data.pop("Proton Intensity")
    ...
    training_labels = training_data.pop(TARGET_NAME)
    test_labels = test_data.pop(TARGET_NAME)
    val_labels = val_data.pop(TARGET_NAME)
