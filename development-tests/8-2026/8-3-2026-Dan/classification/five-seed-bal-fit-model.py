...
MODEL_SAVE_PATH = "saved_models/balanced-fit-model.keras"
MEDIAN_PARAMS_SAVE_PATH = "saved_models/median_params_balanced_fit.json"
MULTI_SEED_RESULTS_SAVE_PATH = "saved_results/multi_seed_results_balanced_fit.json"
TEMP_MODEL_DIRECTORY = "saved_models/five_seed_temp_models"
...
for seed in seeds:
    ...
    temporary_model_path = os.path.join(
        ...
        f"balanced-fit-model-seed-{seed}.keras",
    )
    ...
    # ----------------------------
    # Evaluation at balanced_fit's best decision threshold
    # ----------------------------
    if model.best_decision_threshold is not None:
        ...
        hss_best_threshold = imbal.metrics.HeidkeSkillScore(
            threshold=best_threshold
        )
        hss_best_threshold.update_state(y_test, test_predictions)

        tss_best_threshold = imbal.metrics.TrueSkillStatistic(
            threshold=best_threshold
        )
...
if valid_seed_results:
    ...
    average_f1 = float(
        np.mean(
            [
                result["f1_score_using_best_threshold"]
                for result in valid_seed_results
            ]
        )
    )
    average_hss = float(
        np.mean(
            [
                result["hss_using_best_threshold"]
                for result in valid_seed_results
            ]
        )
    )
    average_tss = float(
        np.mean(
            [
                result["tss_using_best_threshold"]
                for result in valid_seed_results
            ]
        )
    )
...
if valid_seed_results:
    ...
    median_model_data = {
        ...
        "best_weight_index": int(median_result["best_weight_index"]),
        "best_class_weights": [
            float(x) for x in median_result["best_class_weights"]
        ],
...
if average_f1 is not None:
    print(f"Average Test Loss, excluding zero-F1 runs: {average_test_loss:.4f}")
    print(
