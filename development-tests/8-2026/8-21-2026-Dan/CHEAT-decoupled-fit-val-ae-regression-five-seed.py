...
MODEL_SAVE_PATH = "saved_models/CHEAT-decoupled-fit-model-val-ae.keras"
MEDIAN_PARAMS_SAVE_PATH = (
    "saved_models/CHEAT-median_params_decoupled_fit_regression-val-ae.json"
)
MULTI_SEED_RESULTS_SAVE_PATH = (
    "saved_results/CHEAT-multi_seed_results_decoupled_fit_regression-val-ae.json"
)
...
print("\n" + "!" * 72)
print("CHEAT / ORACLE RUN: TEST SET IS USED AS VALIDATION DATA")
print("Epoch selection uses val_loss (MSE); alpha selection uses AORE.")
print("Reported test metrics are oracle/diagnostic values, not unbiased test results.")
print("!" * 72)
...
for seed in seeds:
    ...
    # CHEAT/ORACLE experiment:
    # Train on the full training set and use the test set directly as validation.
    # This intentionally lets test-set performance guide both epoch selection
    # (via val_loss / MSE) and alpha selection (via AORE).
    x_train = x_train_full.copy()
    y_train = y_train_full.copy()
    x_val = x_test.copy()
    y_val = y_test.copy()

    # Fit the KDE on the training labels only.
    labels_kde = y_train.reshape(-1).copy()
    kde = imbal.regression.fit_kde(labels_kde)

    # Candidate sample weights for the full training set.
    train_densities = imbal.regression.get_sample_densities(
        y_train.reshape(-1),
        kde,
    )
    sw_candidates = reciprocal_importance(
        train_densities,
        alpha=alpha_candidates,
    )

    # Matching candidate sample weights for the test-as-validation set,
    # using the KDE fitted from the training labels.
    val_densities = imbal.regression.get_sample_densities(
        y_val.reshape(-1),
        kde,
    )
    sw_val = reciprocal_importance(
        val_densities,
        alpha=alpha_candidates,
    )

    # Alpha candidates are evaluated uniformly so AORE drives candidate choice.
    ...
    temporary_model_path = (
        f"saved_models/tmp_CHEAT-decoupled-fit-model-val-ae-seed-{seed}.keras"
    )
