...
def get_peak_flux(series):
    return float(np.max(series['Proton Intensity'].to_numpy()))

max_fluxes = [get_peak_flux(x) for x in time_series_list]
print(max_fluxes)
ordered_list = np.argsort(max_fluxes)[::-1].tolist()
print(ordered_list)
...
while len(ordered_list) > 0:
    ...
    for i in range(CLUSTER_SIZE):
        if len(ordered_list) > 0:
            new_cluster.append(ordered_list.pop(0))
        else:
            break
...
#
#     intensity_series_one = series_list[index_one]['Proton Intensity'].to_numpy()
#     intensity_series_two = series_list[index_two]['Proton Intensity'].to_numpy()
#
...
for i in range(CLUSTER_SIZE):
    if os.path.exists(f'peak-flux-data/fold-{i+1}'):
        shutil.rmtree(f'peak-flux-data/fold-{i+1}')
    os.mkdir(f'peak-flux-data/fold-{i+1}')
    for value in folds[i]:
        time_series_list[value].to_csv(f'peak-flux-data/fold-{i+1}/sep_event_{value+1}_filled_ie_trim.csv', index=False)