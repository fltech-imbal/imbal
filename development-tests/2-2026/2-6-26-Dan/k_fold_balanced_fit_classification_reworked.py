...
import imbal
from balanced_fit_with_val import balanced_fit_with_val
...
# -------------- Confusion Matrix ---------------
def run_confusion_matrix(model_sep_confmat, y_test_confmat, x_test_confmat):
    y_prob = model_sep_confmat.predict(x_test_confmat, batch_size=32, verbose=0)
    y_pred_temp = (y_prob >= 0.5).astype(int)
    y_true_temp = y_test_confmat.astype(int)
    TP = np.sum((y_true_temp == 1) & (y_pred_temp == 1))
    TN = np.sum((y_true_temp == 0) & (y_pred_temp == 0))
    FP = np.sum((y_true_temp == 0) & (y_pred_temp == 1))
    FN = np.sum((y_true_temp == 1) & (y_pred_temp == 0))
    print("\n=== Confusion Matrix ===")
    print(f"TP: {TP}")
    print(f"FP: {FP}")
    print(f"TN: {TN}")
...
run_confusion_matrix(model, y_test, x_test)
