...
def fit_k_folds_modular(
    ...
    *,
    ...
    # NEW: user-friendly hyperparameter candidates (no grid-building required)
    ...
    Selection rule:
      - For each candidate: score = mean over folds of min(val_loss)
    ...
    for cand_name, cand_params, cand_meta in candidates:
        ...
        for fold, (tr_idx, va_idx) in enumerate(
            ...
            if val_losses:
                ...
                fold_best_val_losses.append(float(np.min(val_losses)))

        # Candidate summary
        avg_best_epoch = int(np.rint(np.mean(fold_best_epochs))) if fold_best_epochs else 1
        avg_best_epoch = max(1, avg_best_epoch)

        # CV score: mean of best val_loss across folds (lower is better)
        cv_score = float(np.mean(fold_best_val_losses)) if fold_best_val_losses else float("inf")

        print(
            f"[CV] candidate={cand_name} | mean_best_val_loss={cv_score:.6f} | avg_best_epoch={avg_best_epoch}"
        )
    ...
    print(f"Selected candidate: {best_name} (mean_best_val_loss={best_cv_score:.6f})")
    ...
    selection_info = {
        "selected_candidate_name": best_name,  # e.g. "alpha=0.5" or "class_weight[2]"
        "selected_alpha": best_meta.get("alpha"),
        "selected_class_weight": best_meta.get("class_weight"),
        "mean_best_val_loss": best_cv_score,
