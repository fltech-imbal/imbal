...
(x_train, y_train), (x_test, y_test) = imbal.regression.split(x_data, y_data, train_size=TRAIN_SPLIT)
...
model = (
    imbal.classification.Model(inputs=resnet.input, outputs=output)
    if MODEL_TASK == 'classification'
    else imbal.regression.Model(inputs=resnet.input, outputs=output)
)
