...
from decoupled_fit_with_cross_val import decoupled_fit_with_cross_val
...
stage_one_params = RegularFitParams(
    x=x_train,
    y=y_train,
...
stage_two_params = BalancedFitParams(
    x=x_train,
    y=y_train,
    sample_weight=sample_weights,
    batch_size=batch_size,
...
stage_one_history, stage_two_history = decoupled_fit_with_cross_val(
    model,
    x=x_train,
    y=y_train,
    batch_size=batch_size,
    ...
    num_folds=5,
    seed=seed,
    stage_one_params=stage_one_params,
    stage_two_params=stage_two_params,
...
plot_auroc_curve(
   ...
   x_test,
   y_test,
   title="Decoupled Fit (Current Model) Test ROC (colored by threshold)",
   color_by_threshold=True,
   save_path="decoupled_fit_current_model_roc_curve.png",
)
# plot_auroc_curve(
#     model,
#     x_test,
#     y_test,
#     title="Decoupled Fit (Paper Model) Test ROC (colored by threshold)",
#     color_by_threshold=True,
#     save_path="decoupled_fit_paper_model_roc_curve.png",
# )
...
f1_threshold_sweep(model.predict(x_test), y_test, 0.1)
