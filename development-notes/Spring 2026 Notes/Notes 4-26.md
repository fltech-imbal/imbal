...
## Tasks
#### High Priority
- LIME, SHAP, and GradCam are separate tutorials $\checkmark$
...
- Documentation now publicly available [here](https://fltech-imbal.github.io/imbal/index.html) $\checkmark$
- Created `fltech-imbal` GitHub organization, transferred ownership of repo $\checkmark$
...
## Tasks
#### High Priority
- In tutorials, change variable `weight_candidates` to `sample_weight_candidates` $\checkmark$
...
- Ask a CS student to help with presentation timing and comments $\checkmark$
- Thorough scan of documentation $\checkmark$
	- Make sure to get tutorial code and documentation draft by Monday $\checkmark$
		...
		- Tutorials based on heliophysics tabular/image data
			- 3x3 tutorials, metrics, t-SNE, LIME, SHAP (tabular) $\checkmark$
		- Installation guide $\checkmark$
- [Keras GradCam](https://keras.io/examples/vision/grad_cam/) how easily can we wrap this and implement in imbal?
	- Make sure it works for binary classification with single output unit (convert to 2 class)
	- Make sure to mention code is taken from that link in documentation
