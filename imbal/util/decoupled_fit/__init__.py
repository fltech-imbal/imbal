...
import imbal.util.helpers as helpers
import numpy as np
import warnings
...
def decoupled_fit(
    ...
    if isinstance(compile_parameters, helpers.TFModelCompileParameters):
        compile_parameters = compile_parameters.to_dict()
    if isinstance(stage_one_compile_parameters, helpers.TFModelCompileParameters):
        stage_one_compile_parameters = stage_one_compile_parameters.to_dict()
    if isinstance(stage_two_compile_parameters, helpers.TFModelCompileParameters):
    ...
    found_layer, found_index = helpers.get_last_trainable_index(
        model,
        n_to_last=2,
